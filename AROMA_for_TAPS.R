...
## Workflow.
## 1. Source this file.
## 2. From the folder containing your samples (sample folders) run TAPS_plot().
...
library('stats')
library('fields')
TAPS_plot <- function(directory=getwd(),xlim=c(-1,2),ylim=c(0,1),minseg=1,maxCn=8,bin=400) {
...
## in Log-R to (autosome) deletion, then update sampleInfo_TAPS.txt.
  ...
  for (i in 1:length(subs)) {
    ...
    cat('..estimating ')
    if (is.null(sampleInfo)) try( sampleInfo <- getEstimates(Log2,alf,allRegions$regions), silent=T) ## Estimates the Log-R of copy number 2 and the difference to a deletion or amplification
...
## This function is currently not in use.  	
  ...
  for (i in 1:length(subs)) {
    ...
    for (chr in 1:24) chromPlot_short(chr, Log2, alf, regs, name)
    cat('..done\n')
    setwd('..')
  }
}
...
## If segmentation is required, DNAcopy is a good choice. Must be installed. 
  library(DNAcopy)
  cat('Using DNAcopy to create segmets:\n')
  segs=NULL
...
## go ahead with TAPS_plot().
    ...
    setwd(CEL_directory)
    cat(nsamples, 'samples processed and stored for TAPS analysis. Next step is to run TAPS_plot().\n')
}
